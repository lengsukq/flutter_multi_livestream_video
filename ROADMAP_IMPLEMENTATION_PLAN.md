...
- 每个新增供应商均以独立可选适配包接入，避免 Chime-only 应用被迫解析 LiveKit、Agora 等 SDK 依赖。
- 同一 `example/` 已注册 Chime、LiveKit 与 Agora Adapter；App 不选择 Provider，由 `demo-server` 返回 Provider。LiveKit 已有真实服务 E2E；Agora 已完成真实项目单会话 join/leave E2E，双设备 host/viewer 与 co-host authentication 强约束仍保留显式 E2E 验证。
...
5. **通用 UI、示例与发布准备 — 已完成当前仓库范围**：复用现有 `example/`，App 不选择 Provider；`demo-server` UI 决定新房间使用 Chime、LiveKit 或 Agora；相关契约与接入文档已补齐。
...
7. **后续供应商适配**：RTC 下一步增加 TRTC、ARTC；直播后续增加 IVS、TRTC、ARTC。每个适配包均实现核心会话、能力声明、事件映射和该供应商必要的渲染适配。
...
- **平台构建**：公共包、Chime、LiveKit、Agora 和示例应通过 Android/iOS 构建，保持当前部署下限；确认相机/麦克风权限配置完整。
